...
public class Compiler {
  ...
  // src/tcljx.compiler/tcljx/data/config.cljt
  ...
  // Problem in short: clojure/core.cljt loads core_print.cljt, which
  // refers via MultiFn.java to functions in clojure.core -- before
  // clojure.core.___ exists.  So track how far translation of the
  // core namespace has progressed, and use the most recent class file
  // for lookups.  The other part of the puzzle is to hack (load
  // "core_print") to flush the current clojure.core namespace segment
  ...
  private static final Class namespaceClass(Lookup l, String ns) {
    ...
    String classPrefix = ns+"._";
    try {
      if (partialCoreNamespace.startsWith(classPrefix)) {
